...
def extract_weight_countvec_sim_feature(args,data,mode="na_v1",sign="train",text='title'):
    if text in ["abstract","title_abstract"]:
        n_components = 128
    else:
        n_components = 64
    savepath = args.featpath + "/{}_{}_{}_weight_countvec_sim_feature.pkl".format(mode,sign,text)
    if not os.path.exists(savepath):
        ...
        for i in tqdm(range(100)):
            ...
            df['{}_wgight_countvec_cosine'.format(text)] = w2v_cosine_list
            print(df['{}_wgight_countvec_cosine'.format(text)])
            df['{}_weight_countvec_cityblock'.format(text)] = w2v_cityblock_list
...
if __name__=="__main__":
    ...
    for mode in ['na_v3']:
        ...
        for text in ['title_abstract','title','abstract','keywords','venue']:
            feat_df = extract_weight_countvec_sim_feature(args,train_df,mode=mode,sign="train",text=text)
            train_feats = pd.merge(train_feats,feat_df,on=['idx'],how='left')
        train_feats = reduce_mem_usage(train_feats)
        train_feats.to_pickle(args.featpath + "/{}_train_weight_countvec_sim_feature.pkl".format(mode))
    ...
    for mode in ['na_v3']:
        ...
        for text in ['title_abstract','title','abstract','keywords','venue']:
            feat_df = extract_weight_countvec_sim_feature(args,test_df,mode=mode,sign="testa",text=text)
            test_feats = pd.merge(test_feats,feat_df,on=['idx'],how='left')
        test_feats = reduce_mem_usage(test_feats)
        test_feats.to_pickle(args.featpath + "/{}_testa_weight_countvec_sim_feature.pkl".format(mode))
    
    for mode in ['na_v3']:
        test_df   = pd.read_pickle(args.featpath + "/{}_testb.pkl".format(mode))
        test_feats = test_df[['idx']]
        for text in ['title_abstract','title','abstract','keywords','venue']:
            feat_df = extract_weight_countvec_sim_feature(args,test_df,mode=mode,sign="testb",text=text)
            test_feats = pd.merge(test_feats,feat_df,on=['idx'],how='left')
        test_feats = reduce_mem_usage(test_feats)
        test_feats.to_pickle(args.featpath + "/{}_testb_weight_countvec_sim_feature.pkl".format(mode))

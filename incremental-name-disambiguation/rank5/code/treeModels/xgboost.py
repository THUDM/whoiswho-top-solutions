...
def kfold_xgboost(args,target,featpath):
    ...
    data_df  = data_df.sample(frac=1,random_state=2021).reset_index(drop=True)
    ...
    for i in range(5):
        ...
        model = XGBClassifier(max_depth=20, learning_rate=args.learning_rate, 
                              n_estimators=args.num_boost_round, subsample=0.8,
                              n_jobs=-1, min_child_weight=6, random_state=2021)
        ...
        model.fit(train_x, train_y,
                  ...
                  verbose = True)
        # 验证集预测结果及评分
        val_pred = model.predict_proba(valid_x)[:, 1]
        ...
        # 测试集预测结果及评分
        testa_pred = model.predict_proba(testa_df[feats])[:, 1]
        ...
        testb_pred = model.predict_proba(testb_df[feats])[:, 1]

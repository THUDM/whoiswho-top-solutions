...
def gen_tfidfvec_features(df, value,n=10):
    df.columns = ['list']
    ...
    vec_svd.columns = ['svd_tfidfvec_{}_{}'.format(value, i) for i in range(n)]
    df = pd.concat([df, vec_svd], axis=1)
    del df['list']
    return df

def extract_weight_tfidfvec_sim_feature(args,data,mode="na_v1",sign="train",text='title'):
    if text in ["abstract","title_abstract"]:
        n_components = 128
    else:
        n_components = 64
    savepath = args.featpath + "/{}_{}_{}_weight_tfidfvec_sim_feature.pkl".format(mode,sign,text)
    if not os.path.exists(savepath):
        # In[]
        # 构建字典，便于查询
        print("Bulding dict")
        text2emb_path = args.outpath + "/{}_tfidfvec_{}.pkl".format(mode,text)
        if not os.path.exists(text2emb_path): 
            ...
            cntvec_feat = gen_tfidfvec_features(text_df[[text]],text,n=n_components)
            cntvec_feat['svd_tfidfvec_{}'.format(text)] = cntvec_feat.apply(lambda xs:[x for x in xs],axis=1)
            text_df = pd.concat([text_df[['paper_id']],cntvec_feat[['svd_tfidfvec_{}'.format(text)]]],axis=1)
            print(text_df['svd_tfidfvec_{}'.format(text)])
                
            text2emb = {}
            for paper_id,vec in tqdm(zip(text_df['paper_id'].values,text_df['svd_tfidfvec_{}'.format(text)].values)):
        ...
        for i in tqdm(range(100)):
            ...
            df['{}_wgight_tfidfvec_cosine'.format(text)] = w2v_cosine_list
            print(df['{}_wgight_tfidfvec_cosine'.format(text)])
            df['{}_weight_tfidfvec_cityblock'.format(text)] = w2v_cityblock_list
...
if __name__=="__main__":
    ...
    for mode in ['na_v3']:
        ...
        for text in ['title_abstract','title','abstract','keywords','venue']:
            feat_df = extract_weight_tfidfvec_sim_feature(args,train_df,mode=mode,sign="train",text=text)
            train_feats = pd.merge(train_feats,feat_df,on=['idx'],how='left')
        train_feats = reduce_mem_usage(train_feats)
        train_feats.to_pickle(args.featpath + "/{}_train_weight_tfidfvec_sim_feature.pkl".format(mode))
    ...
    for mode in ['na_v3']:
        ...
        for text in ['title_abstract','title','abstract','keywords','venue']:
            feat_df = extract_weight_tfidfvec_sim_feature(args,test_df,mode=mode,sign="testa",text=text)
            test_feats = pd.merge(test_feats,feat_df,on=['idx'],how='left')
        test_feats = reduce_mem_usage(test_feats)
        test_feats.to_pickle(args.featpath + "/{}_testa_weight_tfidfvec_sim_feature.pkl".format(mode))
    
    for mode in ['na_v3']:
        test_df   = pd.read_pickle(args.featpath + "/{}_testb.pkl".format(mode))
        test_feats = test_df[['idx']]
        for text in ['title_abstract','title','abstract','keywords','venue']:
            feat_df = extract_weight_tfidfvec_sim_feature(args,test_df,mode=mode,sign="testb",text=text)
            test_feats = pd.merge(test_feats,feat_df,on=['idx'],how='left')
        test_feats = reduce_mem_usage(test_feats)
        test_feats.to_pickle(args.featpath + "/{}_testb_weight_tfidfvec_sim_feature.pkl".format(mode))

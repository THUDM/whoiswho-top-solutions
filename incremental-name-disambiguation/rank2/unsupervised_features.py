...
from build_glove_embeds import load_paper_vectors, load_author_vectors
from data_utils import load_datas, get_author_order_from_valid_papers
from name_utils import process_name, get_abbr_name, get_int_for_name, get_train_name_encoder, get_train_name_papers, \
    get_whole_name_encoder, get_whole_author_orgs, get_author_orgs_tfidf, get_coauthor, is_same_name, clean_orgs, \
    get_all_int_for_name, get_whole_author_keywords, get_author_keywords_tfidf
...
def get_author_name_from_valid_papers(unass_datas, unass_paper_infos, whole_author_infos, unass_type='valid'):
    full_name2int, full_int2author_id, abbr_name2int, abbr_int2author_id, simp_name2int, simp_int2author_id, \
    mirror_abbr_name2int, mirror_abbr_int2author_id, mirror_simp_name2int, mirror_simp_int2author_id = \
        get_whole_name_encoder(whole_author_infos)

    # paper_author_orders = get_author_order_from_valid_papers(unass_datas)
    unass_authors = []
    err_cnt = 0
    for line in unass_datas:
        paper, author_i = line.strip().split('-')

        author = unass_paper_infos[paper]['authors'][int(author_i)]['name']
        full_name_int, abbr_name_int, simp_name_int, mirror_abbr_name_int, mirror_simp_name_int = \
            get_all_int_for_name(author)

        key = str(author_i) + '-' + author
        cur_unass = {key: {}}
        for dict_name in full_name2int:
            if full_name_int == full_name2int[dict_name]:
                cand_author_ids = full_int2author_id[full_name_int][dict_name][dict_name]
                cur_unass[key][dict_name] = cand_author_ids
                break

        if len(cur_unass[key]) == 0:
            for dict_name in abbr_name2int:
                if abbr_name_int == abbr_name2int[dict_name]:
                    for standard_name, cand_author_ids in abbr_int2author_id[abbr_name_int][dict_name].items():
                        cur_unass[key][standard_name] = cand_author_ids

                if mirror_abbr_name_int == abbr_name2int[dict_name]:
                    for standard_name, cand_author_ids in abbr_int2author_id[mirror_abbr_name_int][dict_name].items():
                        cur_unass[key][standard_name] = cand_author_ids

        if len(cur_unass[key]) == 0:
            for dict_name in simp_name2int:
                if simp_name_int == simp_name2int[dict_name]:
                    for standard_name, cand_author_ids in simp_int2author_id[simp_name_int][dict_name].items():
                        cur_unass[key][standard_name] = cand_author_ids

                if mirror_simp_name_int == simp_name2int[dict_name]:
                    for standard_name, cand_author_ids in simp_int2author_id[mirror_simp_name_int][dict_name].items():
                        cur_unass[key][standard_name] = cand_author_ids

        if len(cur_unass[key]) == 0:
            for dict_name in mirror_abbr_name2int:
                if abbr_name_int == mirror_abbr_name2int[dict_name]:
                    for standard_name, cand_author_ids in mirror_abbr_int2author_id[abbr_name_int][dict_name].items():
                        cur_unass[key][standard_name] = cand_author_ids

                if mirror_abbr_name_int == mirror_abbr_name2int[dict_name]:
                    for standard_name, cand_author_ids in mirror_abbr_int2author_id[mirror_abbr_name_int][dict_name].items():
                        cur_unass[key][standard_name] = cand_author_ids

        if len(cur_unass[key]) == 0:
            for dict_name in mirror_simp_name2int:
                if simp_name_int == mirror_simp_name2int[dict_name]:
                    for standard_name, cand_author_ids in mirror_simp_int2author_id[simp_name_int][dict_name].items():
                        cur_unass[key][standard_name] = cand_author_ids

                if mirror_simp_name_int == mirror_simp_name2int[dict_name]:
                    for standard_name, cand_author_ids in mirror_simp_int2author_id[mirror_simp_name_int][dict_name].items():
                        cur_unass[key][standard_name] = cand_author_ids

        if len(cur_unass[key]) == 0:
            err_cnt += 1
        else:
            unass_authors.append((paper, cur_unass))

    print('err_cnt: ', err_cnt)  # 55
    json.dump(unass_authors, open(r'resource/{}_unassCandi.json'.format(unass_type), 'w'), ensure_ascii=False, indent=4)


def get_paper_attr(paper_infos):
    unass_authors = json.load(open(r'resource/unassCandi.json'))
    results = {}
    for paper, paper_author_infos in tqdm(unass_authors.items()):
        results[paper] = {}
        authors = paper_infos[paper]['authors']
        keywords = paper_infos[paper]['keywords']
        for paper_author, candi_authors in paper_infos.items():
            results[paper][paper_author] = {}
            paper_author_idx = int(paper_author.split('-')[0])
            cur_author = authors[paper_author_idx]
            cur_name, cur_org = cur_author['name'], cur_author['org']

            coauthors = deepcopy(authors)
            coauthors.remove(cur_author)
            coauthor_ints = []
            for author in coauthors:
                coauthor_ints.append(get_int_for_name(process_name(author['name'])))
            results[paper][paper_author] = [cur_name, cur_org, candi_authors, coauthor_ints, keywords]
    return results
...
if __name__ == '__main__':
    ...
    official_data_dir = r'IJCAI-21-WhoIsWho-baseline-main/datas'
    ...
    # get_author_name_from_valid_papers(valid_unass, valid_paper_infos, whole_author_infos, unass_type='valid')
    # get_author_name_from_valid_papers(test_unass, test_paper_infos, whole_author_infos, unass_type='test')
    ...
    # pickle.dump(valid_test_author_coauthor_weights,
    ...
    unass_authors = json.load(open(r'resource/unassCandi.json'))
    results = {}
    for paper, paper_infos in tqdm(unass_authors.items()):
        for paper_author, candi_authors in paper_infos.items():
            for standard_name, candidates in candi_authors.items():
                for candidate in candidates:
                    if candidate not in results:
                        results[candidate] = []
                    results[candidate].append(paper)

    print(len(results))  # 3031
    json.dump(results, open(r'results/task1/my_recall.json', 'w'))

    # ============================= 官方召回 =============================
    with open("IJCAI-21-WhoIsWho-baseline-main/datas/proNameAuthorPubs.json", 'r') as files:
        nameAidPid = json.load(files)
    name2author_id = {}
    for name, name_infos in nameAidPid.items():
        if name not in name2author_id:
            name2author_id[name] = []
        author_ids = list(name_infos.keys())
        name2author_id[name].extend(author_ids)

    unass_authors = json.load(open(r'IJCAI-21-WhoIsWho-baseline-main/datas/unassCandi.json'))
    results = {}
    for i in tqdm(range(len(unass_authors))):
        paper, standard_name = unass_authors[i]
        paper = paper.split('-')[0]
        for candidate in name2author_id[standard_name]:
            if candidate not in results:
                results[candidate] = []
            results[candidate].append(paper)

    print(len(results))  # 2647
    json.dump(results, open(r'results/task1/official_recall.json', 'w'))
    """

...
def extract_text_tfidfvec_sim_feature(args,data,mode="na_v1",sign="train",text='title'):
    if text in ["abstract","title_abstract"]:
        n_components = 128
    else:
        n_components = 64
    savepath = args.featpath + "/{}_{}_{}_tfidfvec_sim_feature.pkl".format(mode,sign,text)
    if not os.path.exists(savepath):
        ...
        for i in tqdm(range(100)):
            ...
            df['{}_tfidfvec_cosine_list'.format(text)] = all_w2v_cosine
            df['{}_tfidfvec_cityblock_list'.format(text)] = all_w2v_cityblock
         
            drop_columns = [f for f in df.columns if f not in ['idx']]   
    
            for col in ['{}_tfidfvec_cosine_list'.format(text),'{}_tfidfvec_cityblock_list'.format(text)]:
                print(df[col])
...
if __name__=="__main__":
    ...
    for mode in ['na_v3']:
        for text in ['title_abstract','title','abstract','keywords','venue']:
            ...
            extract_text_tfidfvec_sim_feature(args,train_df,mode=mode,sign="train",text=text)
    ...
    for mode in ['na_v3']:
        for text in ['title_abstract','title','abstract','keywords','venue']:
            ...
            extract_text_tfidfvec_sim_feature(args,test_df,mode=mode,sign="testa",text=text)
            
            test_df = pd.read_pickle(args.featpath + "/{}_testb.pkl".format(mode))
            extract_text_tfidfvec_sim_feature(args,test_df,mode=mode,sign="testb",text=text)
